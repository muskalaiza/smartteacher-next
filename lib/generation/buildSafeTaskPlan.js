...
function getTemplateTaskTypes({
  ...
  if (hasInvalidTaskSubtype) {
    throw new Error(
      `Szablon dla materiału "${materialType}", poziomu "${level}" zawiera nieprawidłowy typ zadania.`
    );
  }
...
export function buildSafeTaskPlan({
  ...
  return {
    ...
    /*
      Pole pozostaje tymczasowo dla zgodności
      z aktualnym kontraktem route.js.

      Po usunięciu coverage z Route Handlera
      usuniemy je razem ze słowem "Safe"
      w nazwie modułu i funkcji.
    */
