...
function buildTaskExplanation(task) {
  if (
    typeof task.answerExplanation === "string" &&
    task.answerExplanation.trim()
  ) {
    return task.answerExplanation.trim();
  }

  if (
    task.taskSubtype === "open_code" ||
    task.taskSubtype === "open_explain"
  ) {
    return null;
  }
