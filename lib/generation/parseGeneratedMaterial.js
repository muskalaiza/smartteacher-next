...
function normalizeGeneratedTaskContract(task) {
  switch (task.taskSubtype) {
    case "open_code":
      return Object.hasOwn(task, "instruction")
        ? task
        : {
            ...task,
            instruction:
...
function normalizeTask(
  ...
  const taskWithNormalizedContract =
    normalizeGeneratedTaskContract(
      taskWithCleanCode
    );

  if (taskWithNormalizedContract.taskSubtype === "open_explain") {
    delete taskWithNormalizedContract.context;
  }

  if (sourceTopicIdSet) {
    if (
      !Array.isArray(
        taskWithNormalizedContract.sourceTopicIds
      ) ||
      taskWithNormalizedContract.sourceTopicIds.length === 0
    ) {
      throw createParseError(
        `Zadanie ${expectedNumber} nie zawiera sourceTopicIds.`
      );
    }

    const normalizedSourceTopicIds =
      taskWithNormalizedContract.sourceTopicIds.map(
        (sourceTopicId, sourceTopicIndex) => {
          const normalizedSourceTopicId =
            normalizeRequiredString(
              sourceTopicId,
              `tasks[${taskIndex}].sourceTopicIds[${sourceTopicIndex}]`
            );

          if (
            !sourceTopicIdSet.has(
              normalizedSourceTopicId
            )
          ) {
            throw createParseError(
              `Zadanie ${expectedNumber} wskazuje temat spoza zakresu sprawdzianu.`
            );
          }

          return normalizedSourceTopicId;
        }
      );
    ...
    if (
      ...
      throw createParseError(
        `Zadanie ${expectedNumber} zawiera powtórzone sourceTopicIds.`
      );
    }
    ...
    delete taskWithNormalizedContract.sourceTopicIds;
  }

  return {
    ...taskWithNormalizedContract,
    question: String(
      getNormalizedQuestion(taskWithNormalizedContract)
    ).trim(),
  };
}

function normalizeSourceTopicIds({
  ...
  if (!isTest) {
    throw createParseError(
      "sourceTopicIds mogą należeć wyłącznie do sprawdzianu."
    );
  }
  ...
  if (
    ...
    throw createParseError(
      "Sprawdzian nie zawiera listy tematów źródłowych."
    );
  }
  ...
  if (
    ...
    throw createParseError(
      "Lista tematów źródłowych sprawdzianu zawiera duplikaty."
    );
  }
...
export function parseGeneratedMaterial(
  ...
  const normalizedSourceTopicIds =
    normalizeSourceTopicIds({
      normalizedMaterialType,
      sourceTopicIds,
    });

  const sourceTopicIdSet =
    normalizedSourceTopicIds
      ? new Set(
          normalizedSourceTopicIds
        )
  ...
  if (sourceTopicIdSet) {
    ...
    if (
      normalizedSourceTopicIds.some(
        (sourceTopicId) =>
          !coveredSourceTopicIds.has(
            sourceTopicId
          )
      )
    ) {
      throw createParseError(
        "Model nie uwzględnił wszystkich tematów źródłowych sprawdzianu."
      );
    }
  }
  ...
  if (normalizedSourceTopicIds) {
    material.sourceTopicIds =
      normalizedSourceTopicIds;
  }
