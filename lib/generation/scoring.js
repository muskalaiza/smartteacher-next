...
function getTaskSubtype(task) {
  const taskSubtype = task?.taskSubtype;

  if (!taskSubtype || !(taskSubtype in TASK_POINTS)) {
    throw new Error(
      `Nieobsługiwany typ zadania w punktacji: ${
        taskSubtype || "[brak]"
      }.`
    );
  }
...
export function getTaskPoints(task) {
  return TASK_POINTS[getTaskSubtype(task)];
}

export function getScoringCriteria(task) {
  switch (getTaskSubtype(task)) {
    case "closed_single":
    case "closed_tf":
      return ["1 pkt - za poprawną odpowiedź"];
    ...
    default:
      throw new Error("Nieobsługiwany typ zadania w kryteriach punktacji.");
  }
}

export function calculateTotalPoints(tasks = []) {
  if (!Array.isArray(tasks)) {
    throw new Error("Lista zadań do obliczenia punktacji musi być tablicą.");
  }
