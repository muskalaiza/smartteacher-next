...
const PRIVATE_RAG_MINIMUM_SIMILARITY = 0.55
  ...
  */
  ...
  const retrieval =
    await searchPrivateDocumentChunks({
      ...
      matchCount:PRIVATE_RAG_MATCH_COUNT,
      apiKey,
    })
  ...
  retrieval.matches.filter(
    (match) =>
      match.similarity >=
      PRIVATE_RAG_MINIMUM_SIMILARITY
  )

const acceptedRetrieval = {
  ...retrieval,
  matchCount:
    PRIVATE_RAG_MATCH_COUNT,
  resultCount:
